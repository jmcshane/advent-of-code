...
fn main() {
    ...
    let filename = &args[1];
    let mut position = Movement{x:0,y:0};
...
impl Summable for Movement {
    ...
    fn add_assign_ref(&mut self, rhs: &Self) {
        self.x.add_assign_ref(&rhs.x);
        self.y.add_assign_ref(&rhs.y);
    }
    fn sub_assign_ref(&mut self, rhs: &Self) {
        self.x.sub_assign_ref(&rhs.x);
        self.y.sub_assign_ref(&rhs.y);
    }
...
fn apply_direction(input: String) -> Movement {
    ...
    match (*split[0]).to_lowercase().as_str() {
        "up" => return Movement{x:0, y: -1 * distance},
        "down" => return Movement{x: 0, y: distance},
        "forward" => return Movement{x: distance, y: 0},
